...
$consulta_score = mysqli_query($conexion, "SELECT score2 FROM usuarios WHERE name='$nombre_usuario'");
if(mysqli_num_rows($consulta_score) > 0){ 
    $fila = mysqli_fetch_assoc($consulta_score);
    $puntaje_maximo = $fila['score2']; 
}
...
if(isset($_POST['puntaje_final'])) {
    ...
    if ($puntaje_final > $puntaje_maximo) {
        ...
        $actualizar_score = mysqli_query($conexion, "UPDATE usuarios SET score2='$puntaje_final' WHERE name='$nombre_usuario'");
        if($actualizar_score) {
